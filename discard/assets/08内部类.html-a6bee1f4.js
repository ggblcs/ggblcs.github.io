import{_ as i,p as n,q as e,a1 as l}from"./framework-96b046e1.js";const s={},d=l(`<p>内部类的特点：</p><ol><li>可以直接访问外部类的所有数据，包括私有数据。</li><li>内部类提供了更好的封装，可以把内部类隐藏在外部类之内，不允许同一个包中的其他类访问该类。</li></ol><p>内部类有：</p><ul><li>成员内部类</li><li>方法内部类</li><li>静态内部类</li><li>匿名内部类</li></ul><h2 id="成员内部类" tabindex="-1"><a class="header-anchor" href="#成员内部类" aria-hidden="true">#</a> 成员内部类</h2><p>外部类不能直接访问内部类的成员和方法，就像一个普通类不能直接访问另一个普通类的成员和方法一样，如果想要访问可以先创建一个==内部类对象==，然后通过内部类对象来访问。 ==此处创建内部类对象有特别之处，请参考下面例子 内部类 对象名 = 外部类对象.new 内部类( );==</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>//外部类HelloWorld
